...
export function renderGithubActionsWorkflow(options?: {
  ...
  const ciReportPath = `${workspacePath}/.fdt/reports/ci-pipeline.json`;
  const sarifPath = `${workspacePath}/.fdt/reports/security-audit.sarif.json`;
  ...
  fdt:
    ...
    steps:
      ...
      - name: Upload CI pipeline report
        if: always() && hashFiles('${ciReportPath}') != ''
